...
/**
 * ========================================================================
 * CUSTOMIZAÇÃO DO MENU LATERAL: ÍCONES PHOSPHOR E REMOÇÃO DE MENUS
 * ========================================================================
 */

// 1. Remover o menu de Comentários
add_action('admin_menu', function () {
    remove_menu_page('edit-comments.php');
});

// 2. Carregar a biblioteca Phosphor no Admin
add_action('admin_enqueue_scripts', function () {
    wp_enqueue_script('phosphor-icons', 'https://unpkg.com/@phosphor-icons/web', [], null, true);
});

// 3. Injetar a substituição de ícones no final da página (Admin Footer)
add_action('admin_footer', function () {
?>
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Mapeamento: ID do menu WordPress -> Classe do Phosphor Icon
            const iconMap = {
                'menu-dashboard': 'ph-squares-four',
                'toplevel_page_googlesitekit-dashboard': 'ph-google-logo',
                'menu-posts': 'ph-push-pin',
                'menu-media': 'ph-image',
                'menu-pages': 'ph-files',
                'menu-posts-voice': 'ph-quotes',
                'menu-posts-member': 'ph-users-three',
                'menu-appearance': 'ph-paint-brush',
                'menu-plugins': 'ph-plug',
                'menu-users': 'ph-users',
                'menu-tools': 'ph-wrench',
                'menu-settings': 'ph-gear',
                'toplevel_page_mlang': 'ph-translate',
                'toplevel_page_litespeed-cache': 'ph-lightning'
            };

            // Percorre o mapa e substitui o HTML interno
            for (const [id, iconClass] of Object.entries(iconMap)) {
                const menu = document.getElementById(id);
                if (menu) {
                    const iconDiv = menu.querySelector('.wp-menu-image');
                    if (iconDiv) {
                        iconDiv.classList.remove('dashicons-before'); // Remove o ícone padrão
                        iconDiv.innerHTML = '<i class="ph-fill ' + iconClass + '"></i>'; // Injeta o Phosphor
                    }
                }
            }

            // Troca também o ícone do botão de "Recolher Menu" (Collapse)
            const collapseBtn = document.getElementById('collapse-button');
            if (collapseBtn) {
                const collapseIcon = collapseBtn.querySelector('.collapse-button-icon');
                if (collapseIcon) {
                    collapseIcon.classList.remove('dashicons-before');
                    collapseIcon.innerHTML = '<i class="ph-bold ph-caret-double-left"></i>';
                }
            }
        });
    </script>
